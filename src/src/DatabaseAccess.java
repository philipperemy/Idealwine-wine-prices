...
public class DatabaseAccess
{
    ...
    public static void f3(DatabaseAccess databaseAccess)
    {
        Calendar dateCal = Calendar.getInstance();
        dateCal.set(Calendar.YEAR, 2013);
        dateCal.set(Calendar.MONTH, Calendar.AUGUST);
        dateCal.set(Calendar.DAY_OF_MONTH, 11);
        System.out.println(dateCal.getTime());
        System.out.println(databaseAccess.getLastQuoteFromDate("DURFORT_VIVENS", "1999", "Bouteille", dateCal.getTime()));
    ...
    public static void f2(DatabaseAccess databaseAccess)
    {
        try
        {
            System.out.println(databaseAccess.getLastQuote("LAFITE_ROTHSCHILD", "1989", "Bouteille"));
        }
    ...
    public String getLastQuoteFromDate(String name, String millesime, String content, Date date)
    {
        String query = "SELECT * FROM `wine_quotes` WHERE name = '" + name + "' " + "AND millesime = '" + millesime + "' AND timestamp < '" + sdf.format(date) + "' " + "ORDER BY timestamp DESC LIMIT 1";
    ...
    public String getLastQuote(String name, String millesime, String content) throws QuoteNotExistException
    {
        String query = "SELECT `quote` FROM `wine_quotes`" + "WHERE `name` = " + "'" + name + "'" + " AND `millesime` = " + "'" + millesime + "'" + " AND `content` = " + "'" + content + "'" + " ORDER BY `timestamp` DESC LIMIT 1 ";
        try
        {
            ResultSet resultSet = connect.createStatement().executeQuery(query);
            while (resultSet.next())
    ...
    {
        String query = "INSERT INTO `wine`.`wine_quotes` (`ID`, `NAME`, `MILLESIME`, `QUOTE`, `CONTENT`, `TIMESTAMP`) VALUES (NULL, '" + name + "', '" + millesime + "', '" + quote + "', '" + content + "', CURRENT_TIMESTAMP);";
        ...
        try
        {
            ...
            {
                System.out.println("( " + name + " , " + millesime + " , " + content + " , " + quote + " EUR ) ... inserting quote");
                connect.createStatement().execute(query);
            }
            else
            {
                System.out.println("( " + name + " , " + millesime + " , " + content + " , " + quote + " EUR ) ... no update");
            }
    ...
    public void readDataBase()
    {
        try
        {
            ...
            connect = DriverManager.getConnection("jdbc:mysql://localhost/wine?" + "user=root&password=");
